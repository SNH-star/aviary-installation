...
curl -fsSL https://github.com/rhysnewell/aviary/raw/main/test/data/wgsim.1.fq.gz -o wgsim.1.fq.gz
curl -fsSL https://github.com/rhysnewell/aviary/raw/main/test/data/wgsim.2.fq.gz -o wgsim.2.fq.gz
# Test conda install
...
chmod g+rw .
bash -c "docker pull ghcr.io/snh-star/aviary:v$AVIARY_VERSION && docker run \
    -e GTDBTK_DATA_PATH=/tmp \
    -e EGGNOG_DATA_DIR=/tmp \
    -e METABULI_DB_PATH=/tmp \
    -v \`pwd\`:/data ghcr.io/snh-star/aviary:v$AVIARY_VERSION assemble \
...
bash -c "apptainer pull docker://ghcr.io/snh-star/aviary:v$AVIARY_VERSION && apptainer run --cleanenv -B \`pwd\`:\`pwd\` \
    --env GTDBTK_DATA_PATH=/tmp \
    --env EGGNOG_DATA_DIR=/tmp \
    --env METABULI_DB_PATH=/tmp \
    aviary_$AVIARY_VERSION.sif assemble \
    -1 \`pwd\`/wgsim.1.fq.gz \
    -2 \`pwd\`/wgsim.2.fq.gz \
    --use-megahit \
    --skip-qc \
